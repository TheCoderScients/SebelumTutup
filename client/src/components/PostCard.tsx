...
export function PostCard({ post }: { post: PostCardType }) {
  return (
    <article className="group rounded-lg border border-white/10 bg-panel/90 p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-white/20 hover:bg-panel">
      <Link to={`/post/${post.id}`} className="block">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`rounded-full border px-2.5 py-1 text-xs ${categoryTone[post.category]}`}>
            {post.category}
          </span>
          <span className="text-xs text-slate-500">oleh {post.displayName}</span>
          <span className="text-xs text-slate-600">•</span>
          <span className="text-xs text-slate-500">{relativeTime(post.createdAt)}</span>
        </div>

        <h2 className="mt-3 line-clamp-2 text-xl font-semibold leading-snug text-white group-hover:text-mint">{post.title}</h2>
        <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-300">{post.body}</p>
      </Link>

      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-4">
        <div className="flex flex-wrap items-center gap-3 text-sm text-slate-400">
          <span className="inline-flex items-center gap-1.5" title="Skor vote">
            <TrendingUp className="h-4 w-4 text-mint" />
            {compactNumber(post.score)}
          </span>
          <span className="inline-flex items-center gap-1.5" title="Komentar">
            <MessageCircle className="h-4 w-4 text-aqua" />
            {compactNumber(post.commentCount)}
          </span>
          <span title="Reaksi">{compactNumber(post.reactionCount)} reaksi</span>
        </div>

        <div className="flex items-center gap-1.5">
          {reactions.map((emoji) => (
            <span key={emoji} className="rounded-md bg-white/5 px-2 py-1 text-xs text-slate-300">
